 ...
 * Copyright notice
 *
 * (c) 2019 Leon Seipp <l.seipp@microeinhundert.com>
 *
 * All rights reserved
 */
...
class EventHandler {
  ...
  /**
   * Setter to set the bebug mode.
   * @param {boolean} debug Enable or disable the debug mode?
   * @returns {void}
   */
  set debug(debug) {
    this._debug = !!debug;
  ...
  executeListenerCallback(event, callback, handlerOptions = {}, paramOptions = {}) {
    ...
    if (handlerOptions.debug || paramOptions.debug || this.debug) this.logExecutionToConsole(event);
  }
  ...
  executeBusCallback(listener, data, callback, handlerOptions = {}, paramOptions = {}) {
    ...
    if (handlerOptions.debug || paramOptions.debug || this.debug) this.logExecutionToConsole(listener, data);
  }
  ...
  listen(elementOrSelector, listener, callback, handlerOptions = {}, paramOptions = {}) {
    ...
    if (handlerOptions.debug || paramOptions.debug || this.debug) {
  ...
  subscribe(listener, callback, handlerOptions = {}, paramOptions = {}) {
    ...
    if (handlerOptions.debug || paramOptions.debug || this.debug) this.logActionToConsole(listener);
  }
...
class EventManager {
  ...
  handleAll(paramOptions = {}) {
    this.handlers.forEach((handlerInstance) => {
      handlerInstance.handle(paramOptions);
    });
  }
  ...
  handleSingle(elementOrSelector, paramOptions = {}) {
    this.handlers
      .filter(handlerInstance => (handlerInstance.handler.element === elementOrSelector) || (handlerInstance.handler.selector === elementOrSelector))
  ...
  attach(elementOrSelector, paramOptions = {}) {
    return elementOrSelector ? this.handleSingle(elementOrSelector, paramOptions) : this.handleAll(paramOptions);
  }
   ...
   * @param {boolean} options.debug Enable the debug mode for all handlers of this event manager instance.
   * @returns {void}
   */
